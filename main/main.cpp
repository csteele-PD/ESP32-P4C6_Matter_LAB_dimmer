...
static const char *TAG = "matter_p4c6_lab";
...
static constexpr char MATTER_PRODUCT_NAME[] = "MatterP4C6Lab";
static constexpr char MATTER_NODE_LABEL[] = "P4 C6 Matter Lab";
...
public:
    ...
    {
        return copy_string(buf, buf_size, "matter-p4c6-lab-001");
    }
    ...
    {
        return copy_string(buf, buf_size, "ESP32-P4 + ESP32-C6");
    }

    CHIP_ERROR GetRotatingDeviceIdUniqueId(chip::MutableByteSpan &unique_id_span) override
    {
        static constexpr uint8_t unique_id[] = {
            0x41, 0x43, 0x4C, 0x59, 0x53, 0x2D, 0x50, 0x34,
            0x43, 0x36, 0x2D, 0x4C, 0x41, 0x42, 0x2D, 0x30,
            0x30, 0x31,
        };
...
static esp_err_t init_external_connection()
{
#if defined(CONFIG_ESP_EXT_CONN_ENABLE) && defined(CONFIG_ESP_HOST_WIFI_ENABLED)
    esp_extconn_config_t ext_config = ESP_EXTCONN_CONFIG_DEFAULT();
    ESP_LOGI(TAG, "initializing P4 external Wi-Fi connection");
    ESP_RETURN_ON_ERROR(esp_extconn_init(&ext_config), TAG, "esp_extconn_init failed");
#endif
    return ESP_OK;
}
...
{
    ESP_LOGI(TAG, "Starting ESP32-P4 + ESP32-C6 Matter lab");
    ...
    ESP_ERROR_CHECK(init_external_connection());
